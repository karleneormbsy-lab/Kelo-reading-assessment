...
import { BookOpen, Star, Users, Zap, CheckCircle } from 'lucide-react'
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#fef9e7' }}>
      ...
      <nav className="border-b border-brand-200/40 px-6 py-3 bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <Link href="/login" className="px-4 py-2 text-brand-700 hover:text-brand-900 font-medium transition-colors">
              Log In
            </Link>
            <Link href="/signup" className="px-4 py-2 bg-brand-700 text-white rounded-xl font-bold hover:bg-brand-800 transition-colors shadow-sm">
              Get Started
            </Link>
          </div>
        </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-2 gap-10 items-center">
        ...
        <div>
          ...
          {/* Assess · Place · Progress · Succeed */}
          <div className="flex flex-wrap gap-2 mb-8">
            {steps.map((s, i) => (
              <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-brand-200 rounded-full text-sm font-semibold text-brand-700 shadow-sm">
                {s.icon} {s.label}
              </span>
            ))}
          </div>
          ...
          <div className="flex gap-4 flex-wrap">
            <Link href="/placement" className="px-8 py-4 bg-brand-700 hover:bg-brand-800 text-white rounded-2xl font-bold text-lg transition-colors shadow-md">
              🎯 Take Placement Test
            </Link>
            <Link href="/signup?role=teacher" className="px-8 py-4 bg-white border-2 border-brand-300 hover:border-brand-500 text-brand-700 rounded-2xl font-bold text-lg transition-colors">
              I'm a Teacher
            </Link>
          </div>
      ...
      <section className="bg-brand-800 py-10">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center gap-8">
          <div className="relative w-52 h-52 flex-shrink-0 rounded-2xl overflow-hidden shadow-2xl">
            <Image
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-wrap gap-4 justify-center md:justify-start mb-4">
              {['Read', 'Learn', 'Grow'].map(word => (
                <span key={word} className="px-5 py-2 bg-gold-400 text-brand-900 font-display font-black text-xl rounded-xl">
                  {word}
                </span>
              ))}
            </div>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20">
        <h2 className="text-3xl font-display font-black text-center text-brand-900 mb-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {levels.map(lvl => (
            <div key={lvl.num} className={`bg-gradient-to-br ${lvl.color} rounded-2xl p-6 text-white shadow-md`}>
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="flex flex-col gap-3">
            {[
              ...
              <div key={num} className="flex gap-4 bg-white rounded-2xl p-4 shadow-sm border border-brand-100">
